...
const EdgeLTCScreen = () => {
  ...
  const checkUserAnswer = () => {
    ...
    console.log('ayyy');
    if (chosenStickers[0] === CubeUtils.edgeLetterToPiece[letterToGuess].mainColor
        && chosenStickers[1] === CubeUtils.edgeLetterToPiece[letterToGuess].subColor) {
      setFeedback('correct');
  ...
  const pickNextLetter = () => {
    console.log(Helper.randomKey(CubeUtils.edgeLetterToPiece));
    // setLetterToGuess('A');
    setLetterToGuess(Helper.randomKey(CubeUtils.edgeLetterToPiece));
    setChosenStickers([]);
